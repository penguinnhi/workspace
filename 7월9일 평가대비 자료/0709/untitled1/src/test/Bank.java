...
public class Bank {
    ...
    //메서드를 정의하시오.
    public List<Client> getOver(int balance){

        List<Client> overClient = new ArrayList<>();

        for (int i=0;i<clientList.size();i++){
            if (balance<clientList.get(i).getAccInfo().getBalance()){

                overClient.add(clientList.get(i));
            }
        }
